...
### 2026-05-28 版本状态标记：当前 `wip/frenet-cadence-reanchor` 不作为稳定基线

当前远端分支头：

```text
wip/frenet-cadence-reanchor @ 03923fd
```

该版本已经通过 seed `001-003` 复测证明不适合作为稳定避障基线：

- seed `001` 在相同随机地图下由旧实验通过变为 `stuck_zero_speed_limit`。
- seed `002` 和 seed `003` 在当前版本下发生碰撞。
- 666 条候选时整帧规划耗时仍在 `0.5-1.0 s` 量级，远高于 20 Hz 规划周期。
- 临时增加纵向速度采样密度到 `v_step=0.60` 没有改善 seed `001`，反而快速碰撞。
- 当前问题不应继续通过增加采样密度解决；轨迹选择、候选保持/复用、保守碰撞检查和 `occupancy.query_path()` 耗时共同导致鲁棒性退化。

处理决定：

- 保留该分支历史用于追溯失败实验和问题定位。
- 不再在该分支上继续作为稳定实现推进。
- 新的恢复工作从 `d3b594c` 建立独立分支，避免影响当前错误分支的历史。
